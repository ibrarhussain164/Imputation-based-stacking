...
suppressPackageStartupMessages({
  ...
  library(missMethods); library(missForest)
})
...
for (prop_idx in seq_along(missing_props)) {
  ...
  for (split_id in seq_len(n_random_splits)) {
    ...
    # Helper: store single-imputation result row
...
# For each classifier × missingness level: stacking AUC minus best individual
# method AUC, split by split. Reports mean diff, resampling interval, win %.
# ---------------------------------------------------------------------------
...
for (model_name in classifier_models) {
  for (prop_val in missing_props[missing_props > 0]) {
    ...
    # Best individual method per split
    ...
    win_prop  <- mean(diff_vec > 0, na.rm = TRUE)
    
    paired_diff_results <- bind_rows(paired_diff_results, data.frame(
      Model              = model_name,
      Missing_Proportion = prop_val,
      Metric             = "AUC",
      n_splits           = n,
      Mean_Diff          = mean_diff,
      SD_Diff            = sd_diff,
